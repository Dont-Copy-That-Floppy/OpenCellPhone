...
void loop() {
  ...
  else if (bufGPRS.substring(0, 6) == "+CMTI:" && bufGPRS.substring(bufGPRS.length() - 2, bufGPRS.length()) == "\r\n")
    newMessage = true;
  ...
  //==========================================================
  if (!displaySleep && !atStartUpScreen && millis() >= (lastInteract + displayTimeout)) {
    startPasscodeLock = millis();
    lastInteract = millis();
    tft.displayOn(false);
    tft.PWM1out(0);
